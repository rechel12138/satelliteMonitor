 ...
 * @since 2019-02-20
 */
@Api(value = "user-api", tags = {"用户角色表"})
@RestController
@Slf4j
@RequestMapping("/api/suserrole")
public class SuserroleController {
    ...
    @PostMapping(value = "listSuserrole", produces = "application/json;charset=UTF-8")
    public String listSuserrole() {
        List map = iSuserroleService.list();
    ...
    @PostMapping(value = "addSuserrole", produces = "application/json;charset=UTF-8")
    public String addSuserrole(Suserrole inputEntity, HttpSession httpSession) {
        try {
            //检查权限
            ...
            inputEntity.setSurAtpid(UUID.randomUUID().toString());
            inputEntity.setSurAtpcreatedatetime(Utils.getNow());
            inputEntity.setSurAtpcreateuser(ls.getSUser().getSuAccount());
            inputEntity.setSurAtplastmodifydatetime(Utils.getNow());
            inputEntity.setSurAtplastmodifyuser(ls.getSUser().getSuAccount());

            iSuserroleService.save(inputEntity);
            return Utils.makeJSONResponseMsg(ResultTo.SUCCESS_STATUS, ResultTo.SUCCESS_STATUS_MSG, null);
        } catch (Exception e) {
    ...
    @PostMapping(value = "updateSuserrole", produces = "application/json;charset=UTF-8")
    public String updateSuserrole(Suserrole inputEntity, HttpSession httpSession) {
        ...
        try {
            ...
            iSuserroleService.updateById(inputEntity);
            return Utils.makeJSONResponseMsg(ResultTo.SUCCESS_STATUS, ResultTo.SUCCESS_STATUS_MSG, null);
        } catch (Exception e) {
            log.error(e.getMessage());
            log.error(e.getLocalizedMessage());
     ...
     * 删除
     */
    @PostMapping(value = "removeSuserrole", produces = "application/json;charset=UTF-8")
    @ApiOperation(value = "根据ids批量删除", notes = "批量删除")
    @ApiImplicitParam(name = "surAtpid", value = "用户ids ，多个用英文逗号分割", required = true, dataType = "String")
    public String removeSuserrole(@RequestParam("surAtpid") String surAtpid) {
        try {
            String[] idsArray = surAtpid.split(",");
            ArrayList<String> arrayList = new ArrayList<String>(Arrays.asList(idsArray));
            iSuserroleService.removeByIds(arrayList);
            return Utils.makeJSONResponseMsg(ResultTo.SUCCESS_STATUS, ResultTo.SUCCESS_STATUS_MSG, null);
        } catch (Exception e) {
            log.error(e.getMessage());
            log.error(e.getLocalizedMessage());
    ...
    public String findByParamEzui(@RequestParam(value = "keyword", defaultValue = "") String keyword,
                                  ...
                                  @RequestParam(value = "sort", defaultValue = "sur_atplastmodifydatetime") String sort,
                                  @RequestParam(value = "order", defaultValue = "desc") String order) {


        Suserrole obj = new Suserrole();
        //筛选字段list
        List list = new ArrayList();

        list.add("sur_fksuserid");
        list.add("sur_fksroleid");


        IPage<Suser> pageResult = null;
        try {
            pageResult = EzuiUtil.getPageResult(page, rows, sort, order, obj, keyword, list, iSuserroleService);
        } catch (Exception e) {
            log.error(e.getMessage());
            log.error(e.getLocalizedMessage());
