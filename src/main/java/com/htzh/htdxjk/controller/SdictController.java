...
import com.htzh.htdxjk.service.ISdictService;
import com.htzh.htdxjk.service.ISdictService;
...
@RestController
@Slf4j
@Api(value = "user-api", tags = {"系统字典"})
@RequestMapping("/api/sdict")
public class SdictController {


    @Autowired
    private ISdictService iSdictService;


    @PostMapping(value = "listSdict", produces = "application/json;charset=UTF-8")
    public String listSdict() {
        List map = iSdictService.list();
    ...
    @PostMapping(value = "addSdict", produces = "application/json;charset=UTF-8")
    public String addSdict(Sdict inputEntity, HttpSession httpSession) {
        try {
            //检查权限
            ...
            inputEntity.setSdAtpid(UUID.randomUUID().toString());
            inputEntity.setSdAtpcreatedatetime(Utils.getNow());
            inputEntity.setSdAtpcreateuser(ls.getSUser().getSuAccount());
            inputEntity.setSdAtplastmodifydatetime(Utils.getNow());
            inputEntity.setSdAtplastmodifyuser(ls.getSUser().getSuAccount());

            iSdictService.save(inputEntity);
            return Utils.makeJSONResponseMsg(ResultTo.SUCCESS_STATUS, ResultTo.SUCCESS_STATUS_MSG, null);
        } catch (Exception e) {
    ...
    @PostMapping(value = "updateSdict", produces = "application/json;charset=UTF-8")
    public String updateSdict(Sdict inputEntity, HttpSession httpSession) {
        ...
        try {
            ...
            iSdictService.updateById(inputEntity);
            return Utils.makeJSONResponseMsg(ResultTo.SUCCESS_STATUS, ResultTo.SUCCESS_STATUS_MSG, null);
        } catch (Exception e) {
            log.error(e.getMessage());
            log.error(e.getLocalizedMessage());
     ...
     * 删除
     */
    @PostMapping(value = "removeSdict", produces = "application/json;charset=UTF-8")
    @ApiOperation(value = "根据ids批量删除", notes = "批量删除")
    @ApiImplicitParam(name = "sdAtpid", value = "用户ids ，多个用英文逗号分割", required = true, dataType = "String")
    public String removeSdict(@RequestParam("sdAtpid") String sdAtpid) {
        try {
            String[] idsArray = sdAtpid.split(",");
            ArrayList<String> arrayList = new ArrayList<String>(Arrays.asList(idsArray));
            iSdictService.removeByIds(arrayList);
            return Utils.makeJSONResponseMsg(ResultTo.SUCCESS_STATUS, ResultTo.SUCCESS_STATUS_MSG, null);
        } catch (Exception e) {
            log.error(e.getMessage());
            log.error(e.getLocalizedMessage());
    ...
    public String findByParamEzui(@RequestParam(value = "keyword", defaultValue = "") String keyword,
                                  ...
                                  @RequestParam(value = "sort", defaultValue = "sd_atplastmodifydatetime") String sort,
        ...
        list.add("sd_type");
        list.add("sd_indexvalue");
        list.add("sd_desc");
        list.add("sd_showvalue");
        list.add("sd_remark");


        IPage<Suser> pageResult = null;
        try {
            pageResult = EzuiUtil.getPageResult(page, rows, sort, order, obj, keyword, list, iSdictService);
        } catch (Exception e) {
            log.error(e.getMessage());
            log.error(e.getLocalizedMessage());
    ...
    public String getOneById(@RequestParam(value = "atpId", defaultValue = "") String atpId) {
        try {
            return Utils.makeJSONResponseMsg(ResultTo.SUCCESS_STATUS, ResultTo.SUCCESS_STATUS_MSG, iSdictService.getById(atpId));
        } catch (Exception e) {
            log.error(e.getMessage());
            log.error(e.getLocalizedMessage());
            return Utils.makeJSONResponseMsg(ResultTo.FAIL_STATUS, ResultTo.FAIL_STATUS_MSG, null);
